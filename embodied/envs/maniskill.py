...
class ManiSkillView(ManiSkill):
    def __init__(self, task, size=(64, 64), view_size=(720, 720), obs_key="image", act_key="action", seed=None, **kwargs):
        hand_camera = dict(
            width=size[0], height=size[1], shader_pack="default")
        base_camera = dict(
            width=view_size[0], height=view_size[1], shader_pack="rt")
    ...
    def _obs(self, raw_obs, reward, is_first=False, is_last=False, is_terminal=False):
        image = raw_obs["sensor_data"]['hand_camera']["rgb"].cpu().reshape(
            (*self.size, 3))
        view = raw_obs["sensor_data"]['base_camera']["rgb"].cpu().reshape(
            (*self.view_size, 3))
